...
{
	/// <summary>
	/// Validates that the first parameter is greater than or equal to the second parameter.
	/// </summary>
	[Serializable]
	public class GreaterOrEqualAttribute : ComparisonValidatorAttribute
	{
		/// <summary>
		...
		public GreaterOrEqualAttribute(string firstParameter, string secondParameter)
		...
		protected override void Validate(object target, int comparisonResult)
		{
			if (comparisonResult < 0)
			{
				ValidationFailed(string.Format(
					...
					"Parameter '{0}' should be greater than or equal to parameter '{1}'",
					FirstParameter,
					SecondParameter
				));
			}
