...
{
	/// <summary>
	/// Validates that the first parameter is greater than the second parameter.
	/// </summary>
	[Serializable]
	public class GreaterAttribute : ComparisonValidatorAttribute
	{
		/// <summary>
		...
		public GreaterAttribute(string firstParameter, string secondParameter)
		...
		protected override void Validate(object target, int comparisonResult)
		{
			if (comparisonResult <= 0)
			{
				ValidationFailed(string.Format(
					...
					"Parameter '{0}' should be greater than parameter '{1}'",
					FirstParameter,
					SecondParameter
				));
			}
