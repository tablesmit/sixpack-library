...
{
	/// <summary>
	/// Validates that the first parameter is less than the second parameter.
	/// </summary>
	[Serializable]
	public class LessAttribute : ComparisonValidatorAttribute
	{
		/// <summary>
		...
		public LessAttribute(string firstParameter, string secondParameter)
		...
		protected override void Validate(object target, int comparisonResult)
		{
			if(comparisonResult >= 0)
			{
				ValidationFailed(string.Format(
                    ...
                    "Parameter '{0}' should be less than parameter '{1}'",
					FirstParameter,
					SecondParameter
				));
			}
